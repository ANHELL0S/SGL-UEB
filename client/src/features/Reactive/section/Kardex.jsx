import { useReport } from '../hook/useCRUD'
import { useDropdown } from '../hook/useDropdown'
import { LuArchive, LuSearch } from 'react-icons/lu'
import { Button } from '../../../components/Button/Button'
import { useAllKarexStore } from '../../../hooks/useKardex'
import { ReportModal } from '../components/Modal/ModalReport'
import { KardexTable } from '../components/Table/KardexTable'
...
export const KardexSection = () => {
	...
	// CRUD
	const { isVisible: showReportModal, openModal: handleReport, closeModal: toggleReportModal } = useReport()
	...
	const renderState = () => {
		...
		if (kardexData?.length === 0) {
			const notFoundProps = !search
				? {
						...
						title: 'Sin registros',
						description: 'Lo sentimos, no se encontró ningún movimiento.',
					}
				: {
		...
		return <KardexTable kardexData={kardexData} toggleDropdown={toggleDropdown} dropdownVisible={dropdownVisible} />
	}
					...
					<section className='flex items-center justify-between flex-wrap lg:flex-nowrap'>
						...
						<div className='flex items-center justify-between gap-4 flex-wrap lg:flex-nowrap'>
							<div className='flex gap-4 flex-wrap'>
								<Button variant='secondary' size='small' onClick={() => handleReport()}>
									Reporte
								</Button>
							</div>
						</div>
