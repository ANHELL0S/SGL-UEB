...
export const ModalCreateSample = ({ onClose, onSuccess, quoteData }) => {
	...
	const modalProps = {
		...
		experiments: quoteData.experiments,
		loading,
		formData,
		onClose,
...
export const ModalUpdateSample = ({ sample, onClose, onSuccess, quoteData }) => {
	const [loading, setLoading] = useState(false)
	const [formData, setFormData] = useState({
		name: sample?.name || '',
...
export const ModalCreateResult = ({ onClose, onSuccess, sample, quoteData }) => {
	const [loading, setLoading] = useState(false)
	const [formData, setFormData] = useState({
		analysis: '',
		result: '',
...
export const ModalUpdateResult = ({ onClose, onSuccess, sample, quoteData }) => {
	const [loading, setLoading] = useState(false)
	const [formData, setFormData] = useState({
...
export const ModalDeleteResult = ({ sample, onClose, onSuccess }) => {
	...
	const modalProps = {
		text: {
			...
			delete: 'Esta acción eliminará permanentemente el registro, y todas sus interacciones.',
			description_a: `Estás a punto de eliminar el resultado`,
			description_b: `INV ${sample?.code_assigned_ueb}.`,
			description_c: '¿Está seguro?',
			buttonCancel: 'No, mantenlo',
			buttonSubmit: 'Ok, eliminar',
