...
export const ConsumptionForm = ({ text, onSubmit, onClose, onChange, loading, formData, quoteData }) => {
	...
	const handleAnalysisChange = analysis => {
		setSelectedAnalysis(prevAnalysis => {
			const isSameAnalysis = prevAnalysis?.id_experiment_parameter === analysis.id_experiment_parameter
	...
	const message = ['Primero debes de asignar los laboratorios']
								...
								<form
									...
									<div className='space-y-4'>
										...
										<div className='space-y-4'>
											...
											<h2 className='text-sm font-medium'>Laboratorio donde se consumió</h2>
											<div className='grid grid-cols-1 gap-4'>
												<div className='grid grid-cols-1 gap-4'>
													{quoteData.labs.length !== 0 ? (
														<>
															{quoteData?.labs?.map(labItem => (
																<div
																	key={labItem?.laboratory?.id_lab}
																	className='rounded-lg flex flex-col justify-center gap-2 cursor-pointer font-medium transition-all ease-in-out duration-300'>
																	<input
																		type='radio'
																		id={`lab-${labItem?.laboratory?.id_lab}`}
																		value={labItem?.laboratory?.id_lab}
																		checked={selectedLab?.id_lab === labItem?.laboratory?.id_lab}
																		onChange={() => handleLabChange(labItem?.laboratory)}
																		className='hidden'
																	/>

																	<label
																		htmlFor={`lab-${labItem?.laboratory?.id_lab}`}
																		className={`p-3 rounded-lg flex flex-col justify-center gap-2 cursor-pointer font-medium transition-all ease-in-out duration-300 ${
																			selectedLab?.id_lab === labItem?.laboratory?.id_lab
																		...
																		<div className='flex flex-col gap-2'>
																			<div className='flex items-start flex-col gap-1'>
																				<span className='break-words line-clamp-1 font-semibold'>
																					{labItem?.laboratory?.name}
																				</span>
																				<div className='text-xs flex items-center gap-1'>
																					<BiSolidUser />
																					<span>{labItem?.laboratory?.laboratory_analyst?.user?.full_name}</span>
																				</div>
																			</div>
																		</div>
																	</label>
														...
														<>
															<Banner messages={message} type='warning' />
														</>
													)}
												</div>
											</div>
									...
									<div>
										...
										<div className='grid grid-cols-1 gap-4'>
											<div className='grid grid-cols-1 gap-4'>
												{quoteData?.experiments?.map(analysisItem => (
													<div
														key={analysisItem?.laboratory?.id_lab}
														className='rounded-lg flex flex-col justify-center gap-2 cursor-pointer font-medium transition-all ease-in-out duration-300'>
														<input
															type='radio'
															id={`analysis-${analysisItem?.experiment?.id_experiment_parameter}`}
															value={analysisItem?.experiment?.id_experiment_parameter}
															checked={
																selectedAnalysis?.id_experiment_parameter ===
																analysisItem?.experiment?.id_experiment_parameter
															}
															onChange={() => handleAnalysisChange(analysisItem?.experiment)}
															className='hidden'
														/>

														<label
															htmlFor={`analysis-${analysisItem?.experiment?.id_experiment_parameter}`}
															className={`p-3 rounded-lg flex flex-col justify-center gap-2 cursor-pointer font-medium transition-all ease-in-out duration-300 ${
																selectedAnalysis?.id_experiment_parameter ===
																analysisItem?.experiment?.id_experiment_parameter
																	? 'dark:bg-yellow-100 dark:text-gray-700 bg-slate-200'
															...
															<div className='flex flex-col gap-2'>
																<div className='flex items-start flex-col gap-1'>
																	<span className='break-words line-clamp-1 font-semibold'>
																		{analysisItem?.experiment?.name}
																	</span>
																	<div className='text-xs flex items-center gap-1'>
																		<span>Categoria: {analysisItem?.experiment?.category?.name}</span>
																	</div>
																</div>
															</div>
														</label>
