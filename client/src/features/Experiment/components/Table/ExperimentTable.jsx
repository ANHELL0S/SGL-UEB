	...
	const columns = useMemo(
		() => [
			...
			},
			{
				...
				accessor: 'precio público',
				Cell: ({ row }) => <span className='break-words line-clamp-1'>{row.original.public_price}</span>,
			},
			{
				Header: 'Precio interno',
				accessor: 'precio interno',
				Cell: ({ row }) => <span className='break-words line-clamp-1'>{row.original.internal_price}</span>,
			},
			{
				Header: 'Estado',
				accessor: 'estado',
				...
				Cell: ({ row }) => (
					<td className='px-2 text-xs py-1.5 relative'>
						<button
							...
							onClick={() => toggleDropdown(row.original.id_experiment)}
							aria-expanded={dropdownVisible === row.original.id_experiment ? 'true' : 'false'}
							aria-label='Toggle dropdown'>
							<div className={`${dropdownVisible === row.original.id_experiment ? 'rotate-180' : ''}`}>
								{dropdownVisible === row.original.id_experiment ? (
		...
		[dropdownVisible, handleOpenUpdatedModal, handleOpenActivedModal, handleOpenDeletedModal]
	)
