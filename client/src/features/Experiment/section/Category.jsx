...
import {
	...
	ModalRestore,
	ModalUpdate,
} from '../components/Form/Category-CRUD'
import { useCreated, useDeleted, useDeletedPermanent, useRestored, useUpdated } from '../hook/useCRUD'
...
export const CategorySection = () => {
		...
		if (categoriesData?.length === 0) {
			const notFoundProps = !search
				? {
						...
						title: 'Sin registros',
						description: 'Lo sentimos, no se encontró ninguna categoria.',
					}
				: {
	...
	return (
		<>
			...
			{showCreatedModal && <ModalCreate onClose={toggleCreatedModal} onSuccess={fetchCategoriesData} />}
			{showUpdatedModal && (
				<ModalUpdate category={selected} onClose={toggleUpdatedModal} onSuccess={fetchCategoriesData} />
			)}
			{showDeletedModal && (
				<ModalDelete category={selected} onClose={toggleDeletedModal} onSuccess={fetchCategoriesData} />
			)}
			{showRestoredModal && (
				<ModalRestore category={selected} onClose={toggleRestoredModal} onSuccess={fetchCategoriesData} />
			)}
