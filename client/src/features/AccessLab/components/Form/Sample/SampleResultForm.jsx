...
export const SampleResultForm = ({ text, onSubmit, onClose, onChange, loading, formData, experiments }) => {
	...
	const messages = ['Selecciona el análisis correspondiente (adscripción).']
										...
										<div className='space-y-4'>
											...
											<div className='grid grid-cols-1 gap-4 max-h-96 overflow-auto'>
												{experiments?.map((exp, index) => {
													const analysisId = exp?.analysis?.experiments_parameter?.id_experiment_parameter
													return (
														<div
															...
															className={`p-3 text-xs space-y-1 dark:bg-gray-700/40 bg-slate-50 rounded-lg cursor-pointer transition-all ease-in-out duration-200 ${
																selectedAnalysis === analysisId
																	? 'dark:bg-yellow-100 text-gray-700'
																	: 'dark:hover:bg-gray-700'
															}`}
															onClick={() => handleSelectAnalysis(analysisId)}>
															<p
																className={`font-bold ${exp?.analysis?.experiments_parameter?.deletedAt ? 'line-through text-red-500' : ''}`}>
																{exp?.analysis?.experiments_parameter?.name}
															</p>
															<p
																className={`${exp?.analysis?.experiments_parameter?.experiments_category?.dele ? 'line-through text-red-500' : ''}`}>
																Categoría: {exp?.analysis?.experiments_parameter?.experiments_category?.name}
															</p>
														</div>
													)
												})}
