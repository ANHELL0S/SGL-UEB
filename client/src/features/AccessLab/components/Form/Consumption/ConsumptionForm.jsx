...
export const ConsumptionForm = ({ text, onSubmit, onClose, onChange, loading, formData, accessData }) => {
								...
								<form
									...
									<div className='space-y-4'>
										...
										<div className='space-y-4'>
											...
											<div className='grid grid-cols-1 gap-4'>
												<div className='grid grid-cols-1 gap-4'>
													{accessData?.labs?.length !== 0 ? (
														<>
															{accessData?.labs?.map(labItem => (
																<div
																		...
																		<div className='flex flex-col gap-2'>
																			<div className='flex items-start flex-col gap-1'>
																				...
																				<div className='text-xs flex items-center gap-1'>
																					<BiSolidUser />
																					<span>{labItem?.lab?.laboratory_analyst?.user?.full_name}</span>
																				</div>
																			</div>
																		</div>
																	</label>
									...
									<div>
										...
										<div className='grid grid-cols-1 gap-4'>
											{accessData?.experiments?.map(analysisItem => {
												// Extraemos el objeto experiments_parameter de analysisItem
												const expParam = analysisItem.analysis.experiments_parameter
												return (
													<div
														key={expParam.id_experiment_parameter}
														className='rounded-lg flex flex-col justify-center gap-2 cursor-pointer font-medium transition-all ease-in-out duration-300'>
														<input
															type='radio'
															id={`analysis-${expParam.id_experiment_parameter}`}
															value={expParam.id_experiment_parameter}
															checked={selectedAnalysis?.id_experiment_parameter === expParam.id_experiment_parameter}
															onChange={() => handleAnalysisChange(expParam)}
															className='hidden'
														/>
														<label
															htmlFor={`analysis-${expParam.id_experiment_parameter}`}
															className={`p-3 rounded-lg flex flex-col justify-center gap-2 cursor-pointer font-medium transition-all ease-in-out duration-300 ${
																selectedAnalysis?.id_experiment_parameter === expParam.id_experiment_parameter
																	? 'dark:bg-yellow-100 dark:text-gray-700 bg-slate-200'
																	: errors?.analysis
																		? 'dark:bg-gray-700/40 text-red-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 hover:bg-slate-100/50'
																		: 'dark:bg-gray-700/30 dark:text-gray-300 dark:hover:bg-gray-700/60 hover:bg-slate-100/50'
															...
															<div className='flex flex-col gap-2'>
																<div className='flex items-start flex-col gap-1'>
																	<span className='break-words line-clamp-1 font-semibold'>{expParam.name}</span>
																	<div className='text-xs flex items-center gap-1'>
																		<span>Categoria: {expParam.experiments_category?.name}</span>
																	</div>
																</div>
															</div>
														</label>
