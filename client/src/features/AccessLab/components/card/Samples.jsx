...
import { BiSolidCalendarAlt, BiSolidEditAlt, BiSolidInfoCircle, BiSolidTrashAlt } from 'react-icons/bi'
import { formatISOToDate } from '../../../../helpers/dateTimeZone.helper'
import { QuoteActionsSample } from '../dropdown/DropdpwnSamples'

export const SamplesCard = ({ accessData }) => {
	const { error, loading, fetchSamples, sampleQuoteData } = useSampleToQuote(accessData?.quote?.id_quote)
	...
	return (
		<>
							...
							{sampleQuoteData?.samples?.map(sample => (
								<div
									...
									<div className='flex items-center justify-between'>
										...
										<QuoteActionsSample sample={sample} fetchData={fetchSamples} accessData={accessData} />
									</div>
			...
			{showCreatedSampleModal && (
				<ModalCreateSample onClose={toggleCreatedSampleModal} onSuccess={fetchSamples} accessData={accessData} />
			)}
			{showAddResultModal && (
				<ModalCreateResult
					sample={selected}
					onClose={closeAddResult}
