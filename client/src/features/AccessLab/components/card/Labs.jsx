...
export const LabsCard = ({ accessData }) => {
	const { loading, error, asignedLabData, fetchData } = useAsignedaLabStore(accessData.id_access)
					...
					<>
						<section className='col-span-1 py-20 space-y-10 '>
							...
							<div className='mt-4 flex items-center justify-center'>
								<Button variant='secondary' size='small' onClick={data => handleOpenModal(data, handleAsignedLab)}>
					...
					<>
						<section className='col-span-1'>
							<div className='flex justify-between pb-4 dark:text-gray-300 font-bold text-lg text-slate-600'>
								<span>Laboratorio/s ({asignedLabData?.data?.totalRecords})</span>
