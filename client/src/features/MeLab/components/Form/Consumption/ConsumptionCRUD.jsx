...
export const ModalCreateConsumption = ({ onClose, onSuccess }) => {
	...
	const handleSubmit = async data => {
		setLoading(true)
		try {
			const response = await ConsumptionReactiveService.createIndependentRequest(data)
...
export const ModalDeleteConsumption = ({ consumption, onClose, onSuccess }) => {
	...
	const handleSubmit = async e => {
		...
		setLoading(true)
		try {
			const response = await ConsumptionReactiveService.deleteIndependentPeramanentRequest(
				consumption?.id_consumption_reactive
			)
			ToastGeneric({ type: 'success', message: response?.data?.message })
			onClose()
			onSuccess()
	...
	const modalProps = {
		text: {
			...
			description_b: `${parseFloat(consumption?.amount).toString()} ${consumption?.reactive?.units_measurement?.unit} ${consumption?.reactive?.name}.`,
